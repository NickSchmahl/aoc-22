...
def compare(leftList, rightList):
    if type(leftList) is list and type(rightList) is list:
        while len(leftList) > 0 and len(rightList) > 0:
            lefty = leftList.pop(0)
            righty = rightList.pop(0)

            if type(lefty) is list and type(righty) is list:
                if len(lefty) == 0 and len(righty) > 0:
                    return True
                elif len(lefty) > 0 and len(righty) == 0:
                    return False
                else:
                    for i in range(min(len(lefty), len(righty))):
                        comp = compare(lefty[i], righty[i])
                        if type(comp) is bool:
                            return comp
            else:
                comp = compare(lefty, righty)
                if type(comp) is bool:
                    return comp
        return len(leftList) == 0
    elif type(leftList) is list:
        rightList = [rightList]
        return compare(leftList, rightList)
    elif type(rightList) is list:
        leftList = [leftList]
        return compare(leftList, rightList)
    else:
        if leftList < rightList:
            return True
        elif rightList < leftList:
            return False
        else:
            return 'BAH'
...
for i in range(len(pairs)):
    leftStr, rightStr = list(pairs[i].split('\n'))
    leftList = eval(leftStr)
    rightList = eval(rightStr)

    comp = compare(leftList, rightList)
    if comp == True:
        sum += i+1
